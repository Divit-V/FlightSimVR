...
public class AircraftPhysics : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        Weather.Initialize(rb.position.y);
    }

    private void FixedUpdate()
    {
        BiVector3 forceAndTorqueThisFrame = 
            CalculateAerodynamicForces(rb.velocity, rb.angularVelocity, Wind.GetWindVelocity(), Weather.GetAirDensity(rb.position.y), 
        ...
        BiVector3 forceAndTorquePrediction = 
            CalculateAerodynamicForces(velocityPrediction, angularVelocityPrediction, Wind.GetWindVelocity(), Weather.GetAirDensity(rb.position.y), 
            rb.worldCenterOfMass);
